/**
 * Cenário 3 - Homologação WOOBA
 * 1 Adulto e 1 Criança | Ida e volta (GRU x GIG) | Nacional | Pagamento faturado
 *
 * Fluxo: RecuperarSistemasPesquisa → Disponibilidade (ida+volta) → Tarifar
...
const OUT_DIR = path.join(__dirname, 'cenario3-adulto-crianca-multiplos-nacional-faturado')
...
;(async () => {
  ...
  console.log('=== Cenário 3: 1 Adulto + 1 Criança | Ida e volta | Nacional GRU×GIG | Faturado ===')
  ...
  // 2. Disponibilidade (ida e volta na mesma chamada)
  ...
  if (!viagensTrecho1.length) throw new Error('Sem voos de ida GRU→GIG')
  if (!viagensTrecho2.length) throw new Error('Sem voos de volta GIG→GRU')
  ...
  console.log(`  Voo ida:   Id=${vooIda.Id}   | Preço: ${vooIda.Preco?.Total}   | Legs: ${classesSelecionadas.length}`)
  console.log(`  Voo volta: Id=${vooVolta.Id} | Preço: ${vooVolta.Preco?.Total} | Legs: ${classesSelecionadasVolta.length}`)
  ...
  console.log('\n✅ Cenário 3 concluído. Arquivos salvos em:')
  console.log(`   ${OUT_DIR}`)
})().catch(err => {
