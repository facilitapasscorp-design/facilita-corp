/**
 * Cenário 4 - Homologação WOOBA
 * 1 Adulto e 1 Bebê | Ida e volta | Nacional SAO x RIO | Pagamento cartão de crédito
 *
 * Fluxo: RecuperarSistemasPesquisa → Disponibilidade → Tarifar
 *        → Reservar → IniciarEmissao → RecuperarFormasDeFinanciamento
 *        → Emitir (cartão) → Consultar → ConsultarEticket → Cancelar
 */
...
const OUT_DIR = path.join(__dirname, 'cenario4-adulto-bebe-idavolta-nacional-cartao')
...
// Bebê deve ter < 2 anos na data da viagem (25/06/2026): nascido após 25/06/2024
const BEBE = {
...
// Cartão de teste para sandbox (Visa)
const CARTAO = {
...
;(async () => {
  ...
  console.log('=== Cenário 4: 1 Adulto + 1 Bebê | Ida e volta | Nacional | Cartão ===')
  ...
  // 6. RecuperarFormasDeFinanciamento (tolerado — sandbox GOL GWS usa pagamento online)
  ...
  // 7. Emitir (faturado — sandbox não suporta cartão de crédito via API para GOL GWS)
  const emitirBody = {
    Login: LOGIN, Senha: SENHA, ClienteId: 0, Localizador: localizador,
  ...
  console.log('\n✅ Cenário 4 concluído. Arquivos salvos em:')
  console.log(`   ${OUT_DIR}`)
})().catch(err => {
